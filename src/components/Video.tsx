...
import {useTimeStampToSeconds} from "../hooks/useTimeStampToSeconds";
...
function Video({onCurrentTimeChange}: Props) {
  const videoRef = useRef<HTMLVideoElement>(null);
  const [currentTime, setCurrentTime] = useState(0);

  // checks the video for the current time
  useEffect(() => {
    if (videoRef.current) {
      videoRef.current.addEventListener("timeupdate", () => {
        ...
        onCurrentTimeChange?.(currTime);
      });
    }
  }, [videoRef]);

  const transcriptArray = useOptimizeTranscript(VIDEO_TRANSCRIPT);
  ...
  return (
    <div className="flex flex-col">
      <div className="my-auto">
        ...
        <h2 className="text-sm">{currentTime} + current time</h2>
        {/* <h4>{useTimeStampToSeconds(`[00:01:40]`)}</h4> */}
        {/* <h4>{parseTimestampedText(VIDEO_TRANSCRIPT)}</h4> */}
      </div>
    </div>
  );
}
